  ...
  return withErrorHandling(async () => {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system:
          '你是菜單辨識引擎。只能輸出純JSON物件，不可有任何前言、說明文字或markdown符號(例如```)。格式:{"storeName": 店名字串或null, "items": [{"name": 品項名稱, "price": 數字, "category": 分類名稱}]}。category請填菜單上原本印刷的分類標題(例如「漢堡類」「蛋餅類」「現烤吐司」「丹麥系列」「貝果系列」「飲料類」等)，同一分類底下的品項category要填一樣的文字；如果整張菜單完全看不出分類，全部填"其他"。若同一品項有多種規格或價格(例如大杯/小杯、半份/全份)，請拆成多個獨立品項並在名稱後方註明規格，這些規格通常屬於同一個category。價格看不清楚或無法判斷時，不要加入該品項。金額只寫數字，不要加NT$或逗號或任何文字。',
        messages: [
          {
            role: "user",
            content: [
